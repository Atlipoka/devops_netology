  ...
  * Создал iam ключ для сервисного аккаунта
   ...
   * Создал CLI профиль
      * ```
        ...
        terraform ACTIVE
        ```
   * Присвоил значения для сервисого аккаунта ``yc config set service-account-key key.json && yc config set cloud-id b1g9tskfs4f4itprsns9 && yc config set folder-id b1gjjlp1h6jc8jeaclal``
